...
function kpi_highcharts($scope, $http, $filter, $httpParamSerializerJQLike){
	...
	$scope.ajax_graph_users_url = 'kpi_dashboard/ajax_graph_users';
	...
	/* Generate Highcharts */
	$( '#kpis_tablist a[href="#kpi_highcharts_tab"]' ).on('show.bs.tab', function (e) {
	...
	$scope.get_highcharts = function (){
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			data : {action: "get_highcharts", csrf_gd : Cookies.get('csrf_gd')},
			url : base_url + "index.php/kpi_dashboard/ajax_highchart",
	...
	*/
	...
	$scope.process_filter_graph = function(unformatted_data){
		...
		$scope.file =  $http({
		        method  : 'POST',
		        url     : 'kpi_dashboard/ajax_filter_graph',
	...
	*/
	...
	$scope.process_filter_gauge = function(unformatted_data){
		...
		$scope.file =  $http({
		        method  : 'POST',
		        url     : 'kpi_dashboard/ajax_filter_gauge',
		        data    :  $httpParamSerializerJQLike(data),
		        headers : {'Content-Type': 'application/x-www-form-urlencoded'} 
		}).success(function(response){
			var result = response.result;
	...
	$("body").on("click", ".hg_gauge_prev_btn", function(){
		...
		$scope.file =  $http({
		        method  : 'POST',
		        url     : 'kpi_dashboard/ajax_filter_gauge_v3',
		        data    :  $httpParamSerializerJQLike(data),
		        headers : {'Content-Type': 'application/x-www-form-urlencoded'} 
		}).success(function(response){
			var result = response.result;
